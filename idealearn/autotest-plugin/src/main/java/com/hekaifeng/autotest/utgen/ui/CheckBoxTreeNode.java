package com.hekaifeng.autotest.utgen.ui;
...
public class CheckBoxTreeNode extends DefaultMutableTreeNode
{
    ...
    public List<List<String>> getSelectedMethodPaths() {
        List<List<String>> methodPaths = new ArrayList<>();
        List<String> methodPath = new ArrayList<>();
        for (int i = 0; i < children.size(); i++) {
            CheckBoxTreeNode methodNode = (CheckBoxTreeNode) children.get(i);
            methodPath.add(methodNode.userObject.toString());
            for (Object child : methodNode.children) {
                CheckBoxTreeNode classNode = (CheckBoxTreeNode) child;
                methodPath.add(classNode.userObject.toString());

                for (Object o : classNode.children) {
                    CheckBoxTreeNode dependMethodNode = (CheckBoxTreeNode) o;
                    methodPath.add(dependMethodNode.userObject.toString());
                    if (dependMethodNode.isSelected) {
                        methodPaths.add(new ArrayList<>(methodPath));
                    }
                    methodPath.remove(methodPath.size() - 1);
                }
                methodPath.remove(methodPath.size() - 1);
            }
            methodPath.remove(methodPath.size() - 1);
